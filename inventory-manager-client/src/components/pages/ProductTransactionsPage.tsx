...
export const ProductTransactionsPage: React.FC = () => {
    ...
    const [formData, setFormData] = useState<{
        ...
        note: string;
    }>({ productId: 0, quantity: '', type: TransactionType.INVENTORY_IN, note: '' });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        await createProductTx({ ...formData, quantity: Number(formData.quantity) } as any);
        setFormData({ productId: 0, quantity: '', type: TransactionType.INVENTORY_IN, note: '' });
        setShowForm(false);
    };
    ...
    return (
        <MainLayout>
            <div className="flex justify-between items-center mb-6">
                <h1 className="text-2xl font-bold text-slate-800">Product Transactions</h1>
                <Button onClick={() => setShowForm(true)}>New Transaction</Button>
            </div>

            <ProductTransactionList />

            <Modal isOpen={showForm} onClose={() => setShowForm(false)} title="New Product Transaction">
