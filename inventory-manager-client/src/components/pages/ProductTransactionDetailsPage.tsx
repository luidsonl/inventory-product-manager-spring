...
export const ProductTransactionDetailsPage: React.FC = () => {
    const { id } = useParams<{ id: string }>();
    const navigate = useNavigate();
    const { transaction, isLoading, error } = useGetProductTransactionsQuery(undefined, {
    ...
    return (
        <MainLayout>
            <div className="flex justify-between items-center mb-6">
                ...
                <Button onClick={() => navigate('/transactions/products')}>Back to List</Button>
            </div>

            <Card>
                <div className="space-y-4">
                    <div>
                        <span className="font-bold block text-sm text-gray-600">Product</span>
                        <div className="text-lg">{transaction.productName || `ID: ${transaction.productId}`}</div>
                    </div>
